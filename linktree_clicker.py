...
class LinktreeClicker:
    ...
    def get_random_proxy(self):
        ...
        proxy_port = "5959"
        proxy_user = "pcOGzMc3Yu-res-ro"
        proxy_pass = "PC_1v6ddsDypghcrUVJb"
    ...
    async def process_linktree(self, url, proxy_info, user_agent):
        ...
        async with async_playwright() as playwright:
            ...
            try:
                ...
                # Launch Chromium browser with proper configuration
    ...
    async def process_individual_link(self, page, context, href, index, total, is_trendyol):
        """Process a single link, opening it in a new tab with tracking verification"""
        new_page = None
        tracking_data = None
        try:
            ...
            if is_trendyol:
                ...
                tracking_requests, remove_listeners, timestamp, link_id = await self.track_network_requests(new_page, href)
                tracking_data = (tracking_requests, remove_listeners, timestamp, link_id)
            ...
            if tracking_data:
                tracking_requests, remove_listeners, timestamp, link_id = tracking_data
                
                # Clean up the listeners
                remove_listeners()
                
                # Save and analyze the tracking data with simplified output
                if is_trendyol:
                    self.save_tracking_data(tracking_requests, timestamp, link_id)
